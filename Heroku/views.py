...
def query(self):
    ...
    with connection.cursor() as cursor:
        cursor.execute("Select movieapi_comments.comments from movieapi_comments WHERE m_id_id = %s",[movie_id])
        row = cursor.fetchall()
        comm = []
    for r in row:
        comm.append(r[0])
...
def filter(self):
    ...
    with connection.cursor() as cursor:
        cursor.execute('Select movieapi_details."imdbRating", movieapi_details."imdbVotes" from movieapi_details WHERE  %s ilike movieapi_details."Title" ',[movie_title])
        row = cursor.fetchall()
        it = iter(row) 
